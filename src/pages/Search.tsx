    ...
    <div className="max-w-7xl mx-auto pb-20 pt-8 px-4">
      ...
      <div className="flex gap-3 mb-12 overflow-x-auto hide-scrollbar pb-2">
        {['All', 'Movies', 'TV Shows', 'Anime', 'Actors', 'Studios', 'Networks'].map((filter) => (
          <button key={filter} className={`px-6 py-2 rounded-full font-bold text-sm transition-all border shadow-lg ${filter === 'All' ? 'bg-primary text-background border-primary hover:bg-primary/90' : 'bg-surface/50 border-white/10 text-white hover:border-white/30 hover:bg-white/5'}`}>
            {filter}
          </button>
        ))}
      </div>
      ...
      {!query ? (
        ...
        <div className="text-center py-20 bg-surface/30 rounded-3xl border border-white/5">
          ...
          <p className="text-text-muted">We couldn't find any titles matching "{query}"</p>
        </div>
      ) : (
        <div>
          <h2 className="text-xl font-bold text-white mb-6 tracking-wide">Top Results ({results.length})</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {results.map((item: any, i) => (
              <motion.div 
                ...
                onClick={() => navigate('/details', { state: { media: { title: item.title, poster: item.poster, backdrop: item.poster, type: item.type } } })}
                key={item.id} 
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent z-10 flex flex-col justify-end p-5 opacity-100 transition-opacity">
                  <span className="text-white font-bold text-base leading-tight block drop-shadow-md line-clamp-2 mb-1">{item.title}</span>
                  <div className="flex items-center justify-between">
                    <span className="text-primary font-bold text-[10px] uppercase tracking-widest">{item.type}</span>
                    <span className="text-text-muted text-[10px]">{item.provider}</span>
                  </div>
                </div>
              </motion.div>
            ))}
